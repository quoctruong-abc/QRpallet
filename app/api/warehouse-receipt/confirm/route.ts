...
export async function POST(request: Request) {
  ...
  if (!profile || !profile.is_active || (profile.role !== "admin" && profile.position !== "warehouse")) {
    return Response.json({ success: false, error: "Không có quyền xác nhận nhập kho." }, { status: 403 });
  }
  ...
  if (!palletIds.length) return Response.json({ success: false, error: "Chưa chọn pallet." }, { status: 400 });

  const supabase = await createClient();
  const { data: palletData, error: palletError } = await supabase.from("pallet_data")
    .select("pallet_id,itemcode,customer,product_name,quantity")
    .in("pallet_id", palletIds).is("effect_to", null).eq("status", "processingWH");
  ...
  const receipt = data[0] as { receipt_id: string; receipt_date: string; total_pallet: number; total_quantity: number };
