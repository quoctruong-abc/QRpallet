...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("confirm_pending_pallets_tracked", {
    ...
    if (error) {
      ...
      console.error("confirm_pending_pallets_tracked failed", error);
      return NextResponse.json({ success: false, error: friendly }, { status: 409 });
    }

    return NextResponse.json({
      success: true,
      pallets: Array.isArray(data) ? data : [],
    });
  } catch (error) {
    console.error("Scan confirm API failed", error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Không thể xác nhận pallet.",
      },
      { status: 500 },
    );
  }
