...
import { PAGE_PERMISSIONS, POSITION_LABELS, POSITION_ROUTES } from "@/lib/routes";
...
const roleRules = [
  {
    ...
    create: "superadmin, admin, user",
    accounts: "Tất cả tài khoản",
    mapping: "Được chỉnh",
    permissions: "Toàn bộ, luôn bypass",
  },
  {
    role: "admin",
    system: "Trong position của mình",
    create: "Chỉ user cùng position",
    accounts: "Chỉ user cùng position",
    mapping: "Không được chỉnh",
    permissions: "Toàn bộ quyền thuộc position",
  },
  {
    role: "user",
    system: "Không có quyền mặc định",
    create: "Không",
    accounts: "Không",
    mapping: "Không",
    permissions: "Phải được cấp từng quyền",
  },
] as const;

function mappingEnabled(
  rows: PositionPageMapping[],
  position: Position,
  path: string,
) {
  const stored = rows.find((row) => row.position === position && row.path === path);
  if (stored) return stored.is_enabled;
  return POSITION_ROUTES[position].includes(path);
}
...
export default async function SuperadminPage() {
  ...
  const [profilesResult, permissionResult, mappingResult] = await Promise.all([
    ...
    adminClient.from("position_page_access").select("position,path,is_enabled,updated_at"),
  ]);
    ...
    <PageShell profile={profile} title="Superadmin control center">
      <div className="hero-row">
        <div>
          ...
          <p className="muted">
            Quản lý tài khoản, mapping position vào trang và permission cụ thể của user.
          </p>
        </div>
      ...
      <section className="panel">
        <div className="section-heading">
          <div>
            <p className="eyebrow">ROLE RULES</p>
            <h2>Quyền cố định theo role</h2>
            <p className="muted small">
              Các rule này là rule hệ thống. Superadmin và admin không cần cấp permission thủ công.
            </p>
          </div>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Position mapping</th>
                <th>Quyền nghiệp vụ</th>
              </tr>
            </thead>
            <tbody>
              {roleRules.map((rule) => (
      ...
      <section className="panel">
        <div className="section-heading">
          <div>
            <p className="eyebrow">POSITION PAGE MAPPING</p>
            <h2>Position nào được vào trang nào</h2>
          </div>
        </div>
        <form action={updatePositionPageAccess}>
            ...
            <table>
              ...
              <tbody>
                {positions.map((position) => (
                  <tr key={position}>
                    <td><strong>{POSITION_LABELS[position]}</strong></td>
                    {pages.map((page) => (
                      <td key={page.path}>
                        <input
                          aria-label={`${POSITION_LABELS[position]} - ${page.label}`}
                          defaultChecked={mappingEnabled(mappingRows, position, page.path)}
          ...
          {mappingResult.error ? (
            <p className="alert alert-error">
              Chưa đọc được bảng position_page_access: {mappingResult.error.message}
            </p>
          ) : null}
          <div style={{ marginTop: "1rem" }}>
            <button className="button button-primary" type="submit">Lưu position mapping</button>
          </div>
        </form>
      </section>
      ...
      <section className="panel">
        <div className="section-heading">
          <div>
            ...
            <h2>Quyền cụ thể của từng tài khoản user</h2>
            <p className="muted small">
              Superadmin và admin dùng quyền cố định theo role. Chỉ user mới cần lưu permission riêng.
            </p>
          </div>
        </div>
        <div className="table-wrap">
              ...
              {users.map((user) => {
                ...
                return (
                  <tr key={user.id}>
                    ...
                    {permissions.map((permission) => {
                      const roleHasPermission =
                        ...
                        (user.role === "admin" && Boolean(
                          user.position && PAGE_PERMISSIONS[pages.find((page) =>
                            PAGE_PERMISSIONS[page.path]?.includes(permission.key),
                          )?.path ?? ""]?.includes(permission.key),
                        ));
                      return (
                        <td key={permission.key}>
                          {isUser ? (
                            ...
                            <input checked={roleHasPermission} disabled readOnly type="checkbox" />
                          )}
                        </td>
                      );
                    })}
                    <td>
                      {isUser ? (
                        <form action={updateUserPermissions} id={`permissions-${user.id}`}>
                          <input name="user_id" type="hidden" value={user.id} />
                          <button className="button button-small button-primary" type="submit">Lưu</button>
                        </form>
                      ) : <span className="muted small">Theo role</span>}
                    </td>
                    <td>
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: "190px" }}>
                        <form action={resetEmployeePassword} style={{ display: "flex", gap: "0.5rem" }}>
                          <input name="user_id" type="hidden" value={user.id} />
                          <input name="password" minLength={8} placeholder="Mật khẩu mới" required type="password" />
                          <button className="button button-small button-secondary" type="submit">Đặt lại</button>
                        </form>
                        {user.id === profile.id ? (
