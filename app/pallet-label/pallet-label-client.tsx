  ...
  return (
    <div className="quantity-progress">
      <div className="quantity-progress-label">
        <strong>{formatNumber(value)}</strong><span>/ {formatNumber(total)}</span>
      </div>
  ...
  return <>
    ...
    <div className="machine-grid">
      {machines.map((machine) => {
        ...
        const hasMissingConfig = machineRows.some((r) => r.quantity_per_pallet === null);
        return <div className="machine-grid-item" key={machine}>
          <button type="button" className={`machine-card machine-card-simple ${active ? "machine-card-active" : ""}`} onClick={() => selectMachine(machine)}>
            <span className="machine-icon machine-icon-large" style={{ position: "relative" }}>
              🏭
              {hasMissingConfig ? (
                <span
                  aria-label="Có itemcode chưa cấu hình số lượng pallet"
                  title="Có itemcode chưa cấu hình số lượng pallet"
                  style={{ color: "#dc2626", fontSize: "1rem", fontWeight: 800, marginLeft: "0.2rem" }}
                >!</span>
              ) : null}
            </span>
              ...
              <tbody>{visibleRows.map((row) => <tr key={`${row.machine}-${row.wo}-${row.itemcode}`}>
                <td>
                  <div className="action-row">
                    <button className="button button-primary button-small" onClick={() => openCreate(row)}>In tem</button>
                    {row.quantity_per_pallet === null ? (
                      <button
                        className="button button-secondary button-small"
                        disabled={updatingItemcode === row.itemcode}
                        onClick={() => updatePalletConfig(row)}
                      >
                        {updatingItemcode === row.itemcode ? "Đang cập nhật..." : "Cập nhật"}
                      </button>
                    ) : null}
                  </div>
                </td>
    ...
    {dialog ? <div className="modal-backdrop" onMouseDown={closeDialog}>
      <div className="modal-card modal-card-wide" onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-heading">
          <div><p className="eyebrow">PALLET</p><h2>{dialog === "merge" ? "Gộp WO" : selectedRow ? `${selectedRow.wo} · ${selectedRow.itemcode}` : "Tính năng pallet"}</h2></div>
        ...
        {dialog === "create" && selectedRow ? <>
          <div className="pallet-choice-grid">
            <label className={`choice-card ${mode === "full" ? "choice-card-active" : ""}`}>
              ...
              <span><strong>Pallet chẵn</strong><small>Lấy số lượng chuẩn</small></span>
            </label>
        ...
        {dialog === "features" ? <>
          ...
          <div className="table-wrap"><table>
            <thead><tr><th>Pallet ID</th><th>WO</th><th>Itemcode</th><th>Số lượng</th><th>Status</th><th>Ngày tạo</th><th>Thao tác</th></tr></thead>
            <tbody>{featurePallets.length ? featurePallets.map((pallet) => <tr key={`${pallet.pallet_id}-${pallet.created_at}`}>
              <td><strong>{pallet.pallet_id}</strong></td><td>{pallet.wo}</td><td>{pallet.itemcode}</td><td>{formatNumber(pallet.quantity)}</td><td>{pallet.status}</td><td>{new Date(pallet.created_at).toLocaleString("vi-VN")}</td>
              <td><div className="action-row">
                <button className="button button-secondary button-small" disabled={pallet.status !== "production"} title={pallet.status !== "production" ? "Chỉ sửa được pallet trạng thái production" : undefined} onClick={() => { setSelectedPallet(pallet); setQuantity(String(pallet.quantity)); setDialog("edit"); }}>Sửa</button>
                <button className="button button-secondary button-small" disabled={pending || pallet.status !== "production"} title={pallet.status !== "production" ? "Chỉ xóa được pallet trạng thái production" : undefined} onClick={() => deletePallet(pallet)}>Xóa</button>
                <button className="button button-primary button-small" onClick={() => printPallet(pallet.pallet_id)}>In lại</button>
              </div></td>
            </tr>) : <tr><td colSpan={7}>Không tìm thấy pallet phù hợp.</td></tr>}</tbody>
          </table></div>
        </> : null}
        ...
        {dialog === "edit" && selectedPallet ? <>
          ...
          <div className="modal-actions"><button className="button button-secondary" onClick={() => setDialog("features")}>Quay lại</button><button className="button button-primary" disabled={pending} onClick={editPallet}>Lưu sửa đổi</button></div>
        </> : null}
